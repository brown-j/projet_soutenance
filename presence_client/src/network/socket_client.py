...
class PresenceSocketClient:
    ...
    def send_attendance(self, id):
        """
        Envoie une notification de présence au serveur Render.
        :param id: Le nom de l'étudiant reconnu (ex: "Joseph Brown")
        """
        if self.sio.connected:
            print(f"📤 Envoi de la présence pour l'étudiant : {id}")
            self.sio.emit('register_attendance', {'id': id, 'status': 'Present'})
        else:
            print(f"⚠️ [Erreur] Impossible d'enregistrer {id}, le réseau est déconnecté.")
